...
*                                        CHANGE PRIORITY OF A TASK
		在运行时动态修改一个任务的优先级，主要涉及到就绪表、事件表的更新和grp变量的更新
		稍微麻烦了点，更新完了如果os正在running则调用OS_Sched()
*
* Description: This function allows you to change the priority of a task dynamically.  Note that the new
*              priority MUST be available.
				该函数允许你动态的(程序运行时)修改一个任务的优先级，注意新设置的优先级必须可用
*
* Arguments  : oldp     is the old priority 原来的旧优先级
*
*              newp     is the new priority 新设置的优先级
*
...
*                                     not exist.
				你传参中的原来优先级不对，没有那个优先级的任务
*              OS_ERR_TASK_NOT_EXIST  if the task is assigned to a Mutex PIP.
				该任务被设置为信号量优先级继承(要先理解ucos信号量和pip的东西)
*********************************************************************************************************
*/
...
#endif
    OS_ENTER_CRITICAL();
// OSTCBPrioTbl数组中存储着各优先级任务的TCB指针，以优先级为index查表即可得到相应优先级
// 的任务的TCB指针。当该任务不存在时指针值为NULL
...
// 确定了oldprio后即可查表得到原来优先级的任务TCB指针，若指针为NULL则说明原来任务不存在
    ptcb = OSTCBPrioTbl[oldprio];
    if (ptcb == (OS_TCB *)0) {                              /* Does task to change exist?              */
        OS_EXIT_CRITICAL();                                 /* No, can't change its priority!          */
        return (OS_ERR_PRIO);
    }
// 如果TCB指针等于OS_TCB_RESERVED(值为1)，说明这个任务当前分配了mutex（互斥锁），这种状态下不能改
// 它的优先级的
...
#endif
// 在优先级数组表中原来优先级位置删除任务TCB指针，重新挂接到新的优先级位置下
    OSTCBPrioTbl[oldprio] = (OS_TCB *)0;                    /* Remove TCB from old priority            */
    OSTCBPrioTbl[newprio] =  ptcb;                          /* Place pointer to TCB @ new priority     */
// 就绪表中如果原来优先级就是非就绪态不用做更改(因为不可能存在原来优先级是非就绪而新指定的
// 优先级是就绪的这种情况，所以原来优先级只要是非就绪，就什么都不用做就行了)
// 而如果原来优先级是就绪态，则需要在就绪表中将原来优先级置为非就绪，然后将新优先级置为
// 就绪态，同时更新OSRdyGrp变量
	y_old                 =  ptcb->OSTCBY;
...
// OSEventTbl的管理和OSRdyTbl的管理方式一样的
// 如果允许MULTI event,则ptcb->OSTCBEventMultiPtr指针指向多个event的数组
// 通过对该数组的遍历可以依次处理各个event，数组结尾为0
#if (OS_EVENT_EN)
...
*              created by an ISR.
				本函数用于ucos2管理任务的运行，任何可以在多任务开始之前创建，或者由一个运行中的任务动态
				的创建，但是不能在中断ISR中创建任务。多任务开始之前指的是main函数中开始的那个地方
*
* Arguments  : task     is a pointer to the task's code 指针任务函数体的函数指针
*
...
*                               }
*                           }
				p_arg是用来给任务函数传参的，也可以不使用传参，根据情况看
*
...
*                       memory locations.
				ptos指向任务栈的起始位置，ucos2的栈方向是可配置的，如果是减栈则ptos指向的起始位置是栈顶
				如果是增栈则ptos指向的起始位置是栈底
*
*              prio     is the task's priority.  A unique priority MUST be assigned to each task and the
*                       lower the number, the higher the priority.
				prio是指定给该任务的优先级，必须唯一，优先级数字越小级别越高
*
...
*              re-activated by creating the deleted task again.
				本函数用来删除一个任务，任务通过调用该函数传自己的prio进去删除自己，删除掉的任务
				会进入休眠状态，并且可以通过再创建一次来重新激活。所以这种删除并不是真正的删除，相应
				的内存也不会被回收。
*
* Arguments  : prio    is the priority of the task to delete.  Note that you can explicitely delete
*                      the current task without knowing its priority level by setting 'prio' to
*                      OS_PRIO_SELF.
				要删除的任务的优先级数字，因为都是任务自己删除自己，所以可以直接传OS_PRIO_SELF即可
				函数会自动查当前任务的prio的。
*
...
* Notes      : 1) To reduce interrupt latency, OSTaskDel() 'disables' the task:
				为了缩减中断延迟以提升实时性，OSTaskDel通过以下三点来删除(禁用)掉本任务
*                    a) by making it not ready   在就绪表中设置为非就绪状态
*                    b) by removing it from any wait lists  从各种event flag链中删除该任务
*                    c) by preventing OSTimeTick() from making the task ready to run.
						通过将该任务移出OSTaskList链表来防止OSTimeTick()函数将该任务置为就绪态
*                 The task can then be 'unlinked' from the miscellaneous structures in uC/OS-II.
						这个任务被删除后就可以从各常用数据结构中去掉了
...
*                 the next instruction following the enable interrupt instruction is ignored.
					在OS_EXIT_CRITICAL()之后调用OS_Dummy()是因为在很多处理器架构中，使能中断指令之后的一条
					指令都是被忽略的，所以给个Dummy过度一下
*              3) An ISR cannot delete a task. 不能在ISR中删除一个任务
...
注意，每个任务都有个全局唯一的索引就是他的prio，在全局数组OSTCBPrioTbl中用prio作为index查表
就能找到这个任务的TCB结构体首地址，从而找到它的一系列信息。
我们删除一个任务时，实际上并没有影响到这个任务在OSTCBPrioTbl数组中占的位置，将来再次创建
这个prio优先级的任务时还是能找回来它的TCB的。所以这里所谓的删除并不是真的删除这个任务的TCB
只是从各种关联链表中将这个任务都移除了而已。
*********************************************************************************************************
*/
...
/*$PAGE*/
	...
	// tcb指针为0说明该prio对应的任务不存在
    if (ptcb == (OS_TCB *)0) {                          /* Task to delete must exist                   */
        OS_EXIT_CRITICAL();
        return (OS_ERR_TASK_NOT_EXIST);
    }
	// tcb指针为1说明该任务当前被互斥访问中(意思就是另外某个任务正在操作这个任务，弄了一半然后
	// 被调度到这里来了，结果这里要删除任务，冲突了，不能这么干)
    if (ptcb == OS_TCB_RESERVED) {                      /* Must not be assigned to Mutex               */
        OS_EXIT_CRITICAL();
        return (OS_ERR_TASK_DEL);
    }

    OSRdyTbl[ptcb->OSTCBY] &= ~ptcb->OSTCBBitX;			// 在就绪表中设置为非就绪态
...
// 从OSTCBList链表中去掉该任务，这样在OSTimeTick函数中就无法遍历到这个任务，也就不会
// 再将这个任务设置为就绪态，这也是这个任务被删除掉的一个表现
...
*					该函数用于恢复一个之前被挂起的任务，这是唯一的一个可以去除之前显式
					挂起的任务的一个函数
...
*              running when the event arrives.
			用这个函数要特别小心，你如果挂起的函数正在等待一个事件(message、信号量、队列等)，则当等待
			的事件来了时，被挂起的任务不会被运行(因为被挂起了)
*********************************************************************************************************
*/
