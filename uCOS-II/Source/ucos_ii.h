...
#ifdef   OS_GLOBALS
#define  OS_EXT		//全局变量定义，OS_EXT空
#else
#define  OS_EXT  extern	  //全局变量声明，
#endif
...
#define  OS_TCB_RESERVED        ((OS_TCB *)1)		// 创建任务前先将相应prio的任务设置为OS_TCB_RESERVED
							// 表明有任务正在创建中
...
#define  OS_STAT_SEM               0x01u    /* Pending on semaphore    信号量挂起                              */
...
// task的extension功能相关特性
    ...
    INT8U            OSTCBStat;  	// 与挂起任务有关           /* Task      status                                        */
    INT8U            OSTCBStatPend;         /* Task PEND status                                        */
    INT8U            OSTCBPrio;             /* Task priority (0 == highest)                            */

    INT8U            OSTCBX; //（rdytbl）由优先级计算出的组中的bit位置 /* Bit position in group  corresponding to task priority   */
...
OS_EXT  INT32U            OSCtxSwCtr;  // 上下文切换次数             /* Counter of number of context switches           */

#if (OS_EVENT_EN) && (OS_MAX_EVENTS > 0)
OS_EXT  OS_EVENT         *OSEventFreeList;  //event 空闲链表指针        /* Pointer to list of free EVENT control blocks    */
OS_EXT  OS_EVENT          OSEventTbl[OS_MAX_EVENTS];  //全局event 数组      /* Table of EVENT control blocks                   */
#endif
...
OS_EXT  INT32U            OSIdleCtrMax; // 1秒内空闲任务计数器可达的最大值  /* Max. value that idle ctr can take in 1 sec.     */
OS_EXT  INT32U            OSIdleCtrRun; // 1秒内空闲任务计数器值  /* Val. reached by idle ctr at run time in 1 sec.  */
OS_EXT  BOOLEAN           OSStatRdy;    //flag变量，指示任务当前状态是否rdy // 是否执行统计任务的标识  /* Flag indicating that the statistic task is rdy  */
OS_EXT  OS_STK            OSTaskStatStk[OS_TASK_STAT_STK_SIZE];  //统计任务的任务栈         /* Statistics task stack          */
#endif

OS_EXT  INT8U             OSIntNesting;        /* Interrupt nesting level  中断嵌套级别，0表示非中断ISR，1表示1层中断                       */

OS_EXT  INT8U             OSLockNesting;       /* Multitasking lock nesting level 多任务嵌套级别                */

OS_EXT  INT8U             OSPrioCur;           /* Priority of current task   当前运行任务的优先级                     */
OS_EXT  INT8U             OSPrioHighRdy;       /* Priority of highest priority task   当前处于就绪态的任务中优先级最高的那个任务的优先级               */
...
OS_EXT  BOOLEAN           OSRunning;  //OS启动尚未完成初始化时为0，初始化完成可以调度了为1                         /* Flag indicating that kernel is running   */

OS_EXT  INT8U             OSTaskCtr;  // 系统创建的任务数    /* Number of tasks created                  */

OS_EXT  volatile  INT32U  OSIdleCtr;   // 空闲时间计数器   /* Idle counter                   */

OS_EXT  OS_STK            OSTaskIdleStk[OS_TASK_IDLE_STK_SIZE];  //空闲任务的任务栈          /* Idle task stack                */


OS_EXT  OS_TCB           *OSTCBCur;    //当前任务的TCB结构体指针             /* Pointer to currently running TCB         */
OS_EXT  OS_TCB           *OSTCBFreeList;  // 指向free list（当前空闲状态TCB链表）单链表的头指针   /* Pointer to list of free TCBs             */
OS_EXT  OS_TCB           *OSTCBHighRdy;  //当前处于就绪态的任务中优先级最高的那个任务的TCB                            /* Pointer to highest priority TCB R-to-R   */
...
OS_EXT  volatile  INT32U  OSTime;   // 以ticks为单位的系统时间    /* Current value of system time (in ticks)         */
#endif
...
extern  INT8U   const     OSUnMapTbl[256];    //常数数组，和计算当前OSPrioHighRdy有关，              /* Priority->Index    lookup table                 */
